...
#"src/main.cpp" using `ruby run-linter.rb > linter-output.txt`
...
linter_flags = ["cppcoreguidelines-*", "bugprone-*", "performance-*",
                "clang-analyzer-*", "portability-*", "readability-*"]
source_files.each do |file|
  puts "\t" + file.upcase
  warnings = `clang-tidy --checks=#{linter_flags.join(",")} #{file} -- std=c++11 #{file}`.split("\n").select!{|line| line.include?(": warning: ")}
